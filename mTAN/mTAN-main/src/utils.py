...
def evaluate(dim, rec, dec, test_loader, args, num_sample=10, device="cuda"):
    ...
    with torch.no_grad():
        for test_batch in test_loader:
            ...
            mae += mean_absolute_error(observed_data, pred_x, observed_mask) * batch
            test_n += batch
    return (mse / test_n), (mae / test_n)
...
def get_physionet_data(args, device, q, flag=1):
    ...
    if not args.classif:
        ...
        #test_dataset_obj = [sample for sample in test_dataset_obj if sample['Gender'] == '0']
        test_dataset = test_dataset_obj[:len(test_dataset_obj)]
