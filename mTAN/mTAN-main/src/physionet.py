...
class PhysioNet(object):
 ...
 Filter test data for 'Gender = 0' if it's test data
    if not self.train:
        self.filter_test_data()
        
#  Modification Start #
  def filter_test_data(self):
      # Assuming `self.data` is a list of patients and each patient is a tuple
      # like (record_id, tt, vals, mask, labels), where 'vals' contains the parameters.
      # We want to filter the test data based on our requirements
      
      filtered_data = []
      for patient in self.data:
          record_id, tt, vals, mask, labels = patient
          
          # Filter 
          age = self.params_dict.get('Age')
          #gender = self.params_dict.get('Gender')
          if age is not None and 60 <= vals[0][age] < 120:
          #if gender is not None and vals[0][gender] == 1:
              filtered_data.append(patient)
      
      # Update the dataset to only include filtered patients 
