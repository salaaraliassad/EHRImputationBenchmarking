...
if __name__ == '__main__':
    ...
    if args.fname is not None:
        ...
        # Added MAE to orginal code #
        test_mse, test_mae = utils.evaluate(dim, rec, dec, test_loader, args, 1)
        print('Test MSE: ', test_mse, 'Test MAE: ', test_mae )
        test_mse, test_mae = utils.evaluate(dim, rec, dec, test_loader, args, 3)
        print('Test MSE: ', test_mse, 'Test MAE: ', test_mae )
        test_mse, test_mae = utils.evaluate(dim, rec, dec, test_loader, args, 10)
        print('Test MSE: ', test_mse, 'Test MAE: ', test_mae )
        test_mse, test_mae = utils.evaluate(dim, rec, dec, test_loader, args, 20)
        print('Test MSE: ', test_mse, 'Test MAE: ', test_mae )
        test_mse, test_mae = utils.evaluate(dim, rec, dec, test_loader, args, 30)
        print('Test MSE: ', test_mse, 'Test MAE: ', test_mae )
        #test_mse, test_mae = utils.evaluate(dim, rec, dec, test_loader, args, 50)
        #('Test MSE: ', test_mse, 'Test MAE: ', test_mae )

    for itr in range(1, args.niters + 1):
        train_loss = 0
        train_n = 0
        avg_reconst, avg_kl, mse, mae = 0, 0, 0, 0
        if args.kl:
        ...
        print('Iter: {}, avg elbo: {:.4f}, avg reconst: {:.4f}, avg kl: {:.4f}, mse: {:.6f}, mae: {:.6f}'
            .format(itr, train_loss / train_n, -avg_reconst / train_n, avg_kl / train_n, mse / train_n, mae / train_n))
        if itr % 10 == 0:
            test_mse, test_mae = utils.evaluate(dim, rec, dec, test_loader, args, 1)
            print('Test Mean Squared Error', test_mse, ' Test Mean Absolute Error: ', test_mae)
        if itr % 10 == 0 and args.save:
            mse, mae = utils.evaluate(dim, rec, dec, test_loader, args, 1)
            torch.save({
                'args': args,
                'epoch': itr,
